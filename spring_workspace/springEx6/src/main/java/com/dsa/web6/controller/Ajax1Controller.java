...
public class Ajax1Controller {
	 ...
	 * Ajax 요청에 대한 응답
	 * @ResponseBody : 
	 * 		일반적인 Spring MVC 요청 처리에서는 데이터를 반환하지 않고, JSP, Thymeleaf와 같은 뷰를 반환
	 * but, 해당 어노테이션이 붙은 메서드는 컨트롤러 메서드가 반환하는 데이터를
	 * HTTP 응답 본문(Body)에 직접 쓰는데 사용
	 * 즉, 뷰를 반환하는게 아니라, 데이터 자체를 클라이언트에게 반환
	 * > 이 과정은 뷰 렌더링을 거치지 않으므로, JSON, XML, 문자열 등의 형식으로 직접 클라이언트에게 전달
	 */
	@ResponseBody
	...
	public ResponseEntity<?> ajaxTest5(@RequestParam("num4") String a,
			@RequestParam("num5") String b) {
		
		/*
		 * ResponseEntity<?>와 @ResponseBody의 차이점
		 * - @ResponseBody는 컨트롤러 메서드의 반환값을 HTTP 응답 본문으로 직렬화하여 반환.
		 * 상태 코드 및 헤더를 변경할 수 없음.
		 * - ResponseEntity<?>는 응답 본문 뿐만 아니라 상태 코드 및 헤더도 함께 설정 가능.
		 * 
		 */
		try {
			...
			// HTTP 응답 상태 코드 400과 에러 메시지를 본문에 리턴
